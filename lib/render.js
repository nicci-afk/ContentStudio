...
import { elevenTts, elevenTtsTimed, heygenGenerate, heygenStatus, heygenQuota, heygenUploadAudio, providerStatus, claudeJson } from './providers.js';
...
// A cache hit refreshes the file's mtime so storage eviction only ever
// removes entries nothing has touched in months.
const touch = (file) => { try { const now = new Date(); fs.utimesSync(file, now, now); } catch { /* best effort */ } };
...
function buildFootagePlan(sources, slot = SLOT) {
  ...
  const distinctSources = new Set(pool.map((e) => e.s)).size;
  ...
  plan.take = (per) => {
    ...
    if (plan.uses.size < distinctSources) {
      // Never show anything twice while an unused source remains.
      let scanned = 0;
      while (plan.uses.has(entry.s) && scanned < plan.pool.length) {
        plan.pos += 1;
        scanned += 1;
        entry = plan.pool[plan.pos % plan.pool.length];
      }
    } else if (plan.pool.length > 1 && entry.s === plan.last) {
      plan.pos += 1;
      entry = plan.pool[plan.pos % plan.pool.length];
    }
    plan.pos += 1;
...
export function activeRenderIds() {
  const ids = [...jobs.values()].filter((j) => j.status === 'running').map((j) => j.id);
  for (const j of clipJobs.values()) if (j.status === 'running') ids.push(j.id);
  return new Set(ids);
}
...
async function runRender(job, { pkg, profile, platformId, script, hookText, voiceId, orientation, avatar, delivery }) {
  ...
  try {
    ...
    // On-camera beats are kept to a few seconds each: she wants the avatar
    // on screen for a moment, then B-roll. Short beats also hold HeyGen usage
    // (and credits) down. Speech past the cap becomes narration over footage.
    const AVATAR_BEAT_SECONDS = 12;
    const capAvatarBeat = (speech) => {
      const s = String(speech || '').trim();
      const words = s.split(' ');
      const maxWords = Math.floor(AVATAR_BEAT_SECONDS * wordsPerSecond);
      if (words.length <= maxWords) return { beat: s, overflow: '' };
      const cut = words.slice(0, maxWords).join(' ');
      const lastEnd = Math.max(cut.lastIndexOf('. '), cut.lastIndexOf('! '), cut.lastIndexOf('? '));
      const splitAt = lastEnd > cut.length * 0.4 ? lastEnd + 1 : cut.length;
      return { beat: s.slice(0, splitAt).trim(), overflow: s.slice(splitAt).trim() };
    };
    // How many slide slots this render will roughly need, so the media
    // pool can be sized to fill every one of them without a repeat.
    const estSeconds = Math.min(
      spec.maxSeconds || Infinity,
      Math.round(speechWhole.split(' ').length / wordsPerSecond) + 20,
    );
    const neededSlots = Math.min(80, Math.ceil(estSeconds / slot) + 2);
    ...
    const narrationKey = (speech) => {
      const styleKey = voiceStyle === DELIVERY.balanced ? '' : `${voiceStyle.stability}|${voiceStyle.similarity}|${voiceStyle.speed || 1}|`;
      return crypto.createHash('sha256').update(`${voiceId}|${styleKey}${speech}`).digest('hex').slice(0, 24);
    };
    let narrIdx = 0;
    const makeNarration = async (speech) => {
      ...
      if (silent) {
        ...
        const cacheKey = narrationKey(speech);
    ...
    // Word-timed cues for a cached avatar clip: the matching narration
    // alignment usually still sits in the narration cache; read it without
    // spending a TTS call. Missing alignment just means estimated cues.
    const cachedAlignment = (speech) => {
      if (silent) return null;
      try { return JSON.parse(fs.readFileSync(path.join(cacheDir, `${narrationKey(speech)}.align.json`), 'utf8')); } catch { return null; }
    };
    ...
    const mediaFallback = !sources.length;

    // Strict variety: when the package's selected media cannot fill every
    // slide slot without repeating, the strongest unused library assets
    // join the pool. Real uploaded footage ranks first, then analyzed
    // high-quality photos, so nothing has to appear twice in one video
    // while unused material exists anywhere in the library.
    let mediaTopUp = 0;
    if (sources.length < neededSlots) {
      const have = new Set(sources.map((s) => s.mediaId));
      const strength = (m) => (m.quality || 0) + (m.analyzed ? 2 : 0) + (m.kind === 'video' && m.hasOriginal ? 3 : 0);
      const extras = records
        .filter((m) => !have.has(m.id) && (m.kind === 'image' || m.kind === 'video'))
        .sort((a, b) => strength(b) - strength(a))
        .slice(0, neededSlots - sources.length);
      if (extras.length) {
        job.step = `gathering your media (adding ${extras.length} more from your library for variety)`;
        const before = sources.length;
        collect(extras.map((m) => m.id));
        mediaTopUp = sources.length - before;
        if (mediaFallback) mediaTopUp = 0;
      }
    }
    if (!sources.length) throw new Error('your media library is empty. Import photos in the Library first');

    const BLUR_FILL = blurFill(W, H);
    ...
    const buildCutoutPart = async (p, files) => {
      ...
      const cues = buildCues(p.speech, p.align || null, duration);
    ...
    // One voice everywhere: avatar beats default to lip-syncing the
    // creator's cloned ElevenLabs narration (audioSource 'narration'), so
    // on-camera and b-roll speech come from the same voice. The HeyGen
    // voice path stays for silent-preview renders or an explicit override.
    const avatarAudio = avatar?.audioSource !== 'heygen' && !silent;
    const avatarReady = !!(avatar?.avatarId && status.heygen && (avatarAudio || avatar?.voiceId));
    ...
    if (allMode) {
      ...
      if (spoken[0]?.mode !== 'avatar') {
        // Script opens on b-roll, so the short hook open still fronts it.
        const hook = cleanScriptForSpeech(hookText).slice(0, 600) || speechWhole.slice(0, 300);
        if (hook) {
          const { beat, overflow } = capAvatarBeat(hook);
          parts.push({ kind: 'avatar', tag: 'open', speech: beat });
          if (overflow) parts.push({ kind: 'broll', tag: 'open-b', speech: overflow });
        }
      }
      // Each on-camera beat stays short (a few seconds on camera), then the
      // rest of that beat's line speaks over B-roll footage.
      spoken.forEach((s, i) => {
        if (s.mode === 'avatar') {
          const { beat, overflow } = capAvatarBeat(s.speech);
          parts.push({ kind: 'avatar', tag: `s${i + 1}`, speech: beat });
          if (overflow) parts.push({ kind: 'broll', tag: `s${i + 1}-b`, speech: overflow });
        } else {
          parts.push({ kind: 'broll', tag: `s${i + 1}`, speech: s.speech });
        }
      });
    } else {
      ...
      if (avatarReady) {
        const hook = cleanScriptForSpeech(hookText).slice(0, 600) || speechWhole.slice(0, 300);
        const { beat } = capAvatarBeat(hook);
        if (beat) {
          parts.push({ kind: 'avatar', tag: 'open', speech: beat });
          if (bodyBudget) bodyBudget = Math.max(10, bodyBudget - Math.round(beat.split(' ').length / wordsPerSecond));
        }
      }
    ...
    const cacheExt = cutout ? 'webm' : 'mp4';
    // Finished HeyGen clips are cached per (avatar, voice, style, orientation,
    // speed, spoken text) in the workspace, so re-rendering the same on-camera
    // beat reuses the clip instead of spending HeyGen credits again. Only the
    // raw avatar clip is cached; it composites over fresh footage each render.
    const avatarCacheDir = path.join(dir, 'avatar-cache');
    // Audio-driven clips key on the narration voice + delivery (the audio
    // carries the pacing); HeyGen-voice clips keep the exact legacy key so
    // every already-paid-for cache entry stays valid.
    const avatarKeyFor = (text) => crypto.createHash('sha256')
      .update(avatarAudio
        ? ['elaudio', avatar?.avatarId, avatar?.avatarKind, voiceId, voiceStyle.stability, voiceStyle.similarity, voiceStyle.speed || 1, cacheExt, portrait ? 'p' : 'l', text].join('|')
        : [avatar?.avatarId, avatar?.avatarKind, avatar?.voiceId, cacheExt, portrait ? 'p' : 'l', voiceStyle.avatarSpeed || 1, text].join('|'))
    ...
    if (avatarParts.length) {
      ...
      job.step = 'requesting avatar sections (HeyGen)';
      // Credit preflight: a drained account degrades every fresh beat to
      // narrated b-roll immediately, instead of failing them one by one
      // in silence after minutes of polling.
      try { heygenCredits = (await heygenQuota()).credits; } catch { heygenCredits = null; }
    }
    const noCredits = heygenCredits != null && heygenCredits <= 0;
    for (const p of avatarParts) {
      // Audio-driven beats are one clip per beat (the uploaded track sets
      // the length); text-driven beats chunk to HeyGen's per-scene cap.
      p.chunks = (avatarAudio ? [p.speech] : chunkSentences(p.speech, 1400).slice(0, 5))
        .map((text) => ({ text, cacheFile: path.join(avatarCacheDir, `${avatarKeyFor(text)}.${cacheExt}`) }));
      try {
        for (const c of p.chunks) {
          if (fs.existsSync(c.cacheFile)) { c.cached = true; touch(c.cacheFile); avatarCached += 1; continue; }
          if (noCredits) throw new Error('no HeyGen credits');
          let audioAssetId;
          if (avatarAudio) {
            job.step = `narrating your on-camera beat (${p.tag})`;
            const narr = await makeNarration(c.text);
            if (narr.alignment) p.align = narr.alignment;
            audioAssetId = await heygenUploadAudio(fs.readFileSync(narr.file));
            job.step = 'requesting avatar sections (HeyGen)';
          }
    ...
    // Gather a part's clip files (copied from cache, or downloaded fresh and
    // then written to the cache) and assemble the finished avatar part.
    const gatherAvatarFiles = async (p) => {
      const files = [];
      for (let ci = 0; ci < p.chunks.length; ci++) {
        const c = p.chunks[ci];
        const local = path.join(tmp, `avatar-${p.tag}-${ci}.${cacheExt}`);
        if (c.cached) fs.copyFileSync(c.cacheFile, local);
        else { await downloadTo(c.url, local); try { fs.copyFileSync(local, c.cacheFile); } catch { /* cache best effort */ } }
        files.push(local);
      }
      return files;
    };
    const assembleAvatarPart = async (p, files) => {
      if (cutout) {
        ...
        p.cues = buildCues(p.speech, p.align || null, p.duration);
      } else {
        const normed = [];
    ...
    // Fully cached avatar beats resolve now, with no HeyGen wait or spend.
    for (const p of avatarParts) {
      if (!p.chunks || !p.chunks.every((c) => c.cached)) continue;
      try {
        job.step = `reusing your saved avatar (${p.tag})`;
        if (avatarAudio && !p.align) p.align = cachedAlignment(p.speech);
        await assembleAvatarPart(p, await gatherAvatarFiles(p));
      } catch { p.chunks = null; p.file = undefined; }
    }

    // 2. Assemble all b-roll parts while HeyGen renders the fresh sections.
    for (const p of parts) {
      if (p.kind !== 'broll') continue;
      Object.assign(p, await buildBrollPart(p.speech, p.tag));
    }

    // 3. Collect the freshly generated avatar sections (cached ones already
    // resolved above); a failed or timed-out section degrades to narrated
    // b-roll instead of sinking a long render.
    const pending = avatarParts.filter((p) => p.chunks && !p.file && p.chunks.some((c) => c.videoId));
    ...
    job.avatar = avatarDone > 0;
    job.avatarSections = sectionsMode ? avatarDone : 0;
    job.avatarStyle = avatarDone > 0 ? (cutout ? 'cutout' : 'full') : undefined;
    job.avatarScope = avatarDone > 0 ? (avatar?.scope || 'open') : undefined;
    job.avatarVoice = avatarDone > 0 ? (avatarAudio ? 'narration' : 'heygen') : undefined;
    job.heygenCredits = heygenCredits ?? undefined;
    job.avatarCached = avatarCached || undefined;
    job.avatarFresh = avatarFresh || undefined;
    job.trimmedToFit = trimmedToFit || undefined;
    job.mediaPool = sources.length;
    job.mediaTopUp = mediaTopUp || undefined;
    writeMeta(job, {
      silent, captions: anyCaptions, timed: anyTimed, mediaFallback,
      mediaPool: sources.length,
      mediaTopUp: mediaTopUp || undefined,
      avatar: avatarDone > 0,
      avatarSections: sectionsMode ? avatarDone : 0,
      avatarFailed: avatarFailed || undefined,
      avatarCached: avatarCached || undefined,
      avatarFresh: avatarFresh || undefined,
      avatarStyle: job.avatarStyle,
      avatarScope: job.avatarScope,
      avatarVoice: job.avatarVoice,
      heygenCredits: heygenCredits ?? undefined,
...
export function listRenders(packageId) {
  ...
  const rows = fs.readdirSync(dir)
    ...
    .map((r) => {
      const p = renderPaths(r.id);
      return { ...r, mp4Bytes: fileSize(p.mp4) || undefined, previewBytes: fileSize(p.preview) || undefined, preview: fs.existsSync(p.preview) };
    });
  // Opportunistic backfill: the first time a package's renders are listed,
  // start building fast previews for its newest finished cuts.
  if (packageId) {
    for (const r of rows.filter((x) => (x.status || 'done') === 'done').slice(0, 4)) enqueuePreview(r.id);
  }
  return rows;
}

// ---- delivery: posters and phone-sized preview streams -------------------
// The full-quality MP4 stays the upload master. The in-app player gets an
// instant poster frame and a lazily built lighter rendition, so the Create
// tab never pulls tens of MB just to show a render row.
...
function renderPaths(id) {
  ...
  const dir = rendersDir();
  return {
    mp4: path.join(dir, `${safe}.mp4`),
    poster: path.join(dir, `${safe}.jpg`),
    preview: path.join(dir, `${safe}.preview.mp4`),
    previewPart: path.join(dir, `${safe}.previewpart.mp4`),
  };
}

export function previewFile(id) {
  const { preview } = renderPaths(id);
  return fs.existsSync(preview) ? preview : null;
}

const posterJobs = new Map();

export async function renderPoster(id) {
  const { mp4, poster } = renderPaths(id);
  if (fs.existsSync(poster)) return poster;
  if (!fs.existsSync(mp4)) return null;
  if (!posterJobs.has(id)) {
    posterJobs.set(id, (async () => {
      const part = `${poster.slice(0, -4)}.posterpart.jpg`;
      try {
        await ffmpeg(['-ss', '1', '-i', mp4, '-frames:v', '1', '-vf', 'scale=640:-2', '-q:v', '4', '-y', part], 60000);
      } catch {
        await ffmpeg(['-i', mp4, '-frames:v', '1', '-vf', 'scale=640:-2', '-q:v', '4', '-y', part], 60000);
      }
      fs.renameSync(part, poster);
      return poster;
    })().finally(() => posterJobs.delete(id)));
  }
  try { return await posterJobs.get(id); } catch { return null; }
}

// ---- chapter-to-clips: every chapter of a finished render becomes a
// vertical short ---------------------------------------------------------
// The master already carries exact chapter offsets and a word-timed SRT, so
// each chapter cuts into a 9:16 blur-fill clip with its caption window
// re-burned. Pure local ffmpeg work: zero provider spend, and one long-form
// render multiplies into a week of Shorts/Reels/TikTok drafts.

const clipJobs = new Map();

function parseSrt(text) {
  const cues = [];
  const t = (h, m, s, ms) => (+h) * 3600 + (+m) * 60 + (+s) + (+ms) / 1000;
  for (const block of String(text || '').split(/\n\s*\n/)) {
    ...
    if (body) cues.push({ start: t(m[1], m[2], m[3], m[4]), end: t(m[5], m[6], m[7], m[8]), text: body });
  }
  return cues;
}

export function startClipsJob(renderId) {
  const meta = renderJob(renderId);
  if (!meta || (meta.status || 'done') !== 'done') throw new Error('that render is not finished (or does not exist)');
  const mp4 = renderFile(renderId, 'mp4');
  if (!mp4) throw new Error('no video file exists for that render');
  const pkg = packageStore.get().items.find((p) => p.id === meta.packageId);
  const rec = pkg?.renders?.[meta.platformId];
  const chapters = (rec?.renderId === renderId ? rec.chapters : null) || [];
  if (chapters.length < 2) throw new Error('that render has no chapter list to cut from');
  const free = diskFree();
  if (free && free.freeBytes < 1024 * 1024 * 1024) throw new Error('not enough disk space to cut clips; run storage cleanup first');
  for (const j of clipJobs.values()) {
    if (j.renderId === renderId && j.status === 'running') return j.id;
  }
  const id = uid();
  const job = { id, renderId, status: 'running', step: 'starting', clips: [], total: chapters.length, createdAt: new Date().toISOString() };
  clipJobs.set(id, job);
  runClips(job, { meta, mp4, chapters }).catch((err) => {
    job.status = 'error';
    job.error = String(err.message).slice(0, 400);
  });
  return id;
}

export function clipsJob(id) {
  return clipJobs.get(id) || null;
}

async function runClips(job, { meta, mp4, chapters }) {
  const dir = rendersDir();
  const tmp = path.join(dir, `tmp-${job.id}`);
  fs.mkdirSync(tmp, { recursive: true });
  const [W, H] = [1080, 1920];
  try {
    const srtFile = renderFile(job.renderId, 'srt');
    const allCues = srtFile ? parseSrt(fs.readFileSync(srtFile, 'utf8')) : [];
    const font = findCaptionFont();
    const total = meta.duration || chapters.at(-1).end || 0;
    for (let i = 0; i < chapters.length; i++) {
      const c = chapters[i];
      job.step = `cutting chapter ${i + 1}/${chapters.length}`;
      const end = c.end || chapters[i + 1]?.start || total || c.start + 45;
      const dur = Math.max(3, end - c.start);
      const cut = path.join(tmp, `cut-${i}.mp4`);
      await ffmpeg([
        '-ss', String(c.start), '-t', dur.toFixed(2), '-i', mp4,
        '-filter_complex', `[0:v]${blurFill(W, H)},fps=25,setsar=1[v]`,
        '-map', '[v]', '-map', '0:a?',
        '-c:v', 'libx264', '-preset', 'veryfast', '-crf', '23', '-pix_fmt', 'yuv420p',
        '-c:a', 'aac', '-b:a', '160k', '-ar', '44100', '-ac', '2',
        '-movflags', '+faststart', '-y', cut,
      ]);
      let final = cut;
      const cues = allCues
        .filter((q) => q.end > c.start + 0.2 && q.start < c.start + dur)
        .map((q) => ({ text: q.text, start: Math.max(0, q.start - c.start), end: Math.min(dur, q.end - c.start) }));
      if (font && cues.length) {
        const assFile = `clipcap-${i}.ass`;
        fs.writeFileSync(path.join(tmp, assFile), buildAss({ cues, W, H, fontFamily: font.family, identity: null }));
        const capped = path.join(tmp, `cut-${i}-cap.mp4`);
        try {
          await ffmpeg([
            '-i', path.basename(cut),
            '-vf', `subtitles=${assFile}:fontsdir=${path.dirname(font.file)}`,
          ...
          final = capped;
        } catch { /* uncaptioned clip still ships */ }
      }
      const out = path.join(dir, `${job.renderId}.clip-${i + 1}.mp4`);
      fs.renameSync(final, out);
      job.clips.push({ n: i + 1, title: c.title, start: c.start, duration: Math.round(dur), bytes: fileSize(out) });
    }
    // The clip list rides the render's meta file so it survives restarts
    // and shows up in every render listing. The live job object (which
    // listings prefer over the file) learns the same list.
    try {
      const metaFile = path.join(dir, `${job.renderId}.json`);
      const stored = JSON.parse(fs.readFileSync(metaFile, 'utf8'));
      stored.clips = job.clips;
      fs.writeFileSync(metaFile, JSON.stringify(stored, null, 2));
    } catch { /* listing still shows the files */ }
    const live = jobs.get(job.renderId);
    if (live) live.clips = job.clips;
    job.step = 'done';
    job.status = 'done';
  } finally {
    fs.rmSync(tmp, { recursive: true, force: true });
...
// Previews build one at a time in the background, never while a real render
// is running and never on a tight disk. A miss just means the player streams
// the full file this visit and the preview is ready for the next one.
let previewChain = Promise.resolve();
const previewQueued = new Set();

export function enqueuePreview(id) {
  const { mp4, preview } = renderPaths(id);
  if (!fs.existsSync(mp4) || fs.existsSync(preview) || previewQueued.has(id)) return;
  previewQueued.add(id);
  previewChain = previewChain.then(async () => {
    try {
      if (fs.existsSync(preview)) return;
      if ([...jobs.values()].some((j) => j.status === 'running')) return;
      const free = diskFree();
      if (free && free.freeBytes < 1536 * 1024 * 1024) return;
      const meta = renderJob(id);
      const landscape = (meta?.orientation || 'portrait') === 'landscape';
      const { previewPart } = renderPaths(id);
      await ffmpeg(['-i', mp4,
        '-vf', landscape ? 'scale=960:-2' : 'scale=-2:960',
        '-c:v', 'libx264', '-preset', 'veryfast', '-crf', '28', '-pix_fmt', 'yuv420p',
        '-c:a', 'aac', '-b:a', '96k', '-movflags', '+faststart', '-y', previewPart], 15 * 60 * 1000);
      fs.renameSync(previewPart, preview);
      console.log(`render ${id}: fast preview ready (${Math.round(fileSize(preview) / 1e6)}MB of ${Math.round(fileSize(mp4) / 1e6)}MB)`);
    } catch (err) {
      console.warn(`render ${id}: preview generation failed: ${err.message}`);
    } finally {
      previewQueued.delete(id);
    }
  });
}
