...
function handlesFor(id) {
  if (!handles.has(id)) {
    ...
    handles.set(id, {
      state: jsonFile(path.join(dir, 'state.json'), structuredClone(EMPTY_STATE)),
      media: jsonFile(path.join(dir, 'media.json'), { items: [] }),
...
export const mediaStore = {
  get: () => active().media.get(),
  set: (v) => active().media.set(v),
  update(fn) { return this.set(fn(this.get())); },
  flush: () => active().media.flush(),
};
...
// ---- media files (scoped to the active workspace) ------------------------

export function mediaPath(id, kind) {
  const safe = String(id).replace(/[^a-z0-9_-]/gi, '');
  return path.join(wsDir(activeId()), 'media', `${safe}.${kind}`);
}
