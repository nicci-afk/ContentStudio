...
import { uid, readMediaFile, mediaPath, workspaceDir, mediaStore } from './store.js';
  ...
  try {
    ...
    if (voiceId && status.elevenlabs) {
      ...
      if (fs.existsSync(cachedAudio)) {
        fs.copyFileSync(cachedAudio, narration);
        ...
        if (timed) {
          ...
          fs.writeFileSync(narration, await elevenTts({ voiceId, text: speech, stability: voiceStyle.stability, similarity: voiceStyle.similarity }));
        }
        try { fs.copyFileSync(narration, cachedAudio); } catch { /* cache only */ }
      }
    } else {
      silent = true;
    ...
    // 2. Collect the package's selected media at its strongest stored form:
    ...
    // 3. Slideshow: each slide encodes as its own small segment, then the
    // segments concat losslessly — bounded memory however long the video.
    // Video sources play for real (muted, looped when shorter than their
    // slot); stills get the slow zoom. Identical encode settings keep the
    // lossless concat valid across both.
    ...
    const slides = Array.from({ length: slideCount }, (_, i) => sources[i % sources.length]);
    const per = duration / slides.length;
    const usedClips = new Set();
    const segs = [];
    const encodeStill = (file, seg) => ffmpeg([
    ...
    for (let i = 0; i < slides.length; i++) {
      job.step = `assembling b-roll (${i + 1}/${slides.length})`;
      const slide = slides[i];
      const seg = path.join(tmp, `seg-${i}.mp4`);
      if (slide.type === 'video') {
        try {
          await ffmpeg([
            '-stream_loop', '-1', '-i', slide.file,
            '-t', per.toFixed(3),
            '-filter_complex', `[0:v]${BLUR_FILL},fps=25,setsar=1[v]`,
            '-map', '[v]', ...SEG_ENCODE, '-y', seg,
          ]);
          usedClips.add(slide.mediaId);
        } catch {
          // An undecodable upload falls back to its stored poster frame so
          // the render still completes.
          const buf = readMediaFile(slide.mediaId, 'render') || readMediaFile(slide.mediaId, 'analysis') || readMediaFile(slide.mediaId, 'thumb');
          if (!buf) throw new Error(`could not decode video "${path.basename(slide.file)}" and no preview frame is stored for it`);
          const poster = path.join(tmp, `poster-${i}.jpg`);
          fs.writeFileSync(poster, buf);
          const composed = path.join(tmp, `poster-${i}-fill.png`);
          await ffmpeg(['-i', poster, '-filter_complex', `[0:v]${BLUR_FILL}[v]`, '-map', '[v]', '-frames:v', '1', '-y', composed]);
          await encodeStill(composed, seg);
        }
      } else {
        await encodeStill(slide.file, seg);
      }
      segs.push(seg);
    }
    job.step = 'laying narration under b-roll';
    const listFile = path.join(tmp, 'segs.txt');
    ...
    await ffmpeg([
      ...
      '-map', '0:v', '-map', '1:a',
      '-c:v', 'copy', '-c:a', 'aac', '-b:a', '160k', '-shortest', '-movflags', '+faststart', '-y', slideshow,
    ]);
    ...
    if (avatar?.avatarId && avatar?.voiceId && status.heygen) {
      ...
      const { videoId } = await heygenGenerate({
        avatarId: avatar.avatarId, avatarKind: avatar.avatarKind, voiceId: avatar.voiceId, text: hook,
      ...
      const download = await fetch(avatarUrl);
      if (!download.ok || !download.body) throw new Error('HeyGen video download failed');
      const { Readable } = await import('node:stream');
      const { pipeline } = await import('node:stream/promises');
      await pipeline(Readable.fromWeb(download.body), fs.createWriteStream(avatarFile));
