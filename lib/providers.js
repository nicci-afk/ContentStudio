...
// Bulk callers (media analysis fan-out especially) can run enough parallel
// requests to trip Anthropic's rate limit. Rather than fail the caller on
// the first 429, back off and retry: honor Retry-After when Anthropic sends
// one, otherwise capped exponential backoff with jitter. Real 5xx hiccups
// get the same treatment. Five attempts tops out at well under a minute of
// total wait, so a genuinely broken request still fails promptly.
const RETRYABLE_MAX_ATTEMPTS = 5;
const RETRYABLE_CAP_MS = 20000;

async function sleep(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

// tier 'light' routes utility calls (chapter titles and similar small
// jobs) to a cheaper model when ANTHROPIC_MODEL_LIGHT is set; content
// generation always stays on the main model. Unset, everything behaves
// exactly as before.
export async function claude({ system, messages, maxTokens = 4096, temperature = 1, tier }) {
  const key = process.env.ANTHROPIC_API_KEY;
  if (!key) throw new ProviderError('anthropic', 401, 'ANTHROPIC_API_KEY not configured');
  const main = process.env.ANTHROPIC_MODEL || 'claude-sonnet-5';
  const body = JSON.stringify({
    model: tier === 'light' ? (process.env.ANTHROPIC_MODEL_LIGHT || main) : main,
    max_tokens: maxTokens,
    // Newer models reject temperature overrides ("deprecated for this
    // model"), so the neutral default stays out of the request entirely.
  ...
  for (let attempt = 0; attempt < RETRYABLE_MAX_ATTEMPTS; attempt++) {
    ...
    if (retryable && attempt < RETRYABLE_MAX_ATTEMPTS - 1) {
      const retryAfter = Number(res.headers.get('retry-after'));
      const delay = Number.isFinite(retryAfter) && retryAfter > 0
        ? retryAfter * 1000
        : Math.min(RETRYABLE_CAP_MS, 1000 * 2 ** attempt) + Math.random() * 500;
      await sleep(delay);
      continue;
    }
    throw new ProviderError('anthropic', res.status, (await res.text()).slice(0, 500));
  }
}
...
export function heygenAvatars() {
  return cachedList('heygen-avatars', async () => {
    ...
    // an avatar here under the exact name they gave it in HeyGen. The old
    // v2 list fallback is gone: v2 shuts down on 2026-10-31 and the v3
    // looks list is the only one that carries newer avatar generations.
    ...
    if (!looks.length) throw new ProviderError('heygen', 502, 'the avatar list came back empty; check the HeyGen account');
    return looks;
  });
}

// Remaining credit balance, read before avatar renders so a drained account
// degrades loudly up front instead of failing every section in silence.
// The three billing shapes each keep the number somewhere else.
export function heygenQuota() {
  return cachedList('heygen-quota', async () => {
    const data = await getJsonWithRetry('heygen', `${HEYGEN_URL}/v3/users/me`, heygenHeaders());
    const d = data.data || data;
    let credits = null;
    if (d.usage_based?.remaining_credits != null) credits = Number(d.usage_based.remaining_credits);
    else if (d.subscription?.credits) {
      credits = Number(d.subscription.credits.premium_credits?.remaining || 0)
        + Number(d.subscription.credits.add_on_credits?.remaining || 0);
    } else if (d.wallet?.remaining_balance != null) credits = Number(d.wallet.remaining_balance);
    return { credits: Number.isFinite(credits) ? credits : null, billingType: d.billing_type || null };
  }, 5 * 60 * 1000);
}

// Uploads narration audio (MP3/WAV, 32MB cap upstream) and returns the
// asset id that /v3/videos accepts as the avatar's voice track.
export async function heygenUploadAudio(buffer, filename = 'narration.mp3', mime = 'audio/mpeg') {
  const form = new FormData();
  form.append('file', new Blob([buffer], { type: mime }), filename);
  const res = await fetch(`${HEYGEN_URL}/v3/assets`, { method: 'POST', headers: heygenHeaders(), body: form });
  ...
  if (!id) throw new ProviderError('heygen', 502, 'asset upload returned no id');
  return id;
}
...
// An audioAssetId (from heygenUploadAudio) replaces script + voice_id
// entirely: the avatar lip-syncs the uploaded track, which is how every
// video speaks in the creator's one cloned narration voice.
export async function heygenGenerate({ avatarId, voiceId, text, texts, audioAssetId, title, orientation = 'portrait', speed, transparent }) {
