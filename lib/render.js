...
async function runRender(job, { pkg, profile, platformId, script, hookText, voiceId, orientation, avatar, delivery }) {
  ...
  try {
    ...
    const avatarKeyFor = (text) => crypto.createHash('sha256')
      .update([avatar?.avatarId, avatar?.avatarKind, avatar?.voiceId, cacheExt, portrait ? 'p' : 'l', voiceStyle.avatarSpeed || 1, text].join('|'))
      .digest('hex').slice(0, 24);
    const avatarParts = parts.filter((p) => p.kind === 'avatar');
    let avatarFailed = 0;
    let avatarCached = 0;
    let avatarFresh = 0;
    if (avatarParts.length) { fs.mkdirSync(avatarCacheDir, { recursive: true }); job.step = 'requesting avatar sections (HeyGen)'; }
    for (const p of avatarParts) {
      p.chunks = chunkSentences(p.speech, 1400).slice(0, 5)
        .map((text) => ({ text, cacheFile: path.join(avatarCacheDir, `${avatarKeyFor(text)}.${cacheExt}`) }));
      try {
        for (const c of p.chunks) {
          if (fs.existsSync(c.cacheFile)) { c.cached = true; avatarCached += 1; continue; }
