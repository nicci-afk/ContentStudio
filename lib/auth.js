...
// ---- rate limiting -------------------------------------------------------
// Failed credential attempts are counted per client IP; successful requests
// never count, so heavy legitimate API use over Basic auth is unaffected.
// Everything is in-memory: a restart clears it, which is fine for a limiter
// whose only job is making brute force impractical.

const buckets = new Map(); // key -> { n, resetAt }

function tooMany(key, max) {
  const b = buckets.get(key);
  return !!b && b.resetAt > Date.now() && b.n >= max;
}

function recordFailure(key, windowMs = 15 * 60 * 1000) {
  const now = Date.now();
...
export function authMiddleware(req, res, next) {
  ...
  const hasBasic = (req.headers.authorization || '').startsWith('Basic ');
  if (hasBasic) {
    const key = `basic:${clientIp(req)}`;
    if (tooMany(key, 25)) return res.status(429).json({ error: 'too many failed sign-in attempts; wait a few minutes' });
...
export function registerAuthRoutes(app, publicDir) {
  ...
  app.post('/auth/magic/request', async (req, res) => {
    try {
      ...
      // Per-IP send cap: the response never varies, so a flood burns nothing
      // (no email spend, no address probing) and a real user is untouched.
      const ipKey = `magic:${clientIp(req)}`;
      if (tooMany(ipKey, 12)) {
        return res.json({ ok: true, message: 'If that email is registered, a link is on its way.' });
      }
      recordFailure(ipKey, 3600 * 1000);
  ...
  app.post('/auth/password', (req, res) => {
    ...
    const key = `pw:${clientIp(req)}`;
    if (tooMany(key, 10)) return res.status(429).json({ error: 'too many attempts; wait a few minutes' });
