...
async function runRender(job, { pkg, profile, platformId, script, hookText, voiceId, orientation, avatar }) {
  ...
  try {
    ...
    // which is what keeps a 512MB instance alive through a full long-form.
    ...
    for (let i = 0; i < slides.length; i++) {
      ...
      await ffmpeg([
        '-loop', '1', '-t', per.toFixed(3), '-i', slides[i],
        '-vf',
        `scale=${W}:${H}:force_original_aspect_ratio=increase,crop=${W}:${H},fps=25,` +
        `zoompan=z='min(zoom+0.0008,1.15)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=1:s=${W}x${H}:fps=25,setsar=1`,
