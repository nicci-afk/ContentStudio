...
8. ONE CTA per asset.
9. HUMAN VOICE. Match the creator's voice DNA exactly — their rhythm, vocabulary, and stance. No AI-isms ("dive in", "game-changer", "unleash", "in today's fast-paced world", em-dash chains, "it's not X, it's Y" constructions).
10. STORY SPINE. Every asset carries story: a person, a tension, a turn, a takeaway — even a 15-second Short.`;
...
{
  "faq": [{"q": "conversational question exactly as someone would ask an AI assistant", "a": "45-70 word answer-first response with a number or named entity"}],  // exactly 4 pairs
...
export async function generatePackage({ profile, topic, angle, pillar, series, media = [], platformIds, onProgress }) {
  const status = providerStatus();
  const system = masterContext(profile, { pillar, series, media });
...
export async function synthesizeBrief(answers, profile) {
  ...
  return claudeJson({
    ...
    messages: [{
      role: 'user',
      content: `Here are a creator's storytelling interview answers as JSON:\n${JSON.stringify(answers, null, 2)}\n\nSynthesize their Story Brief. Respond with ONLY JSON:
{
