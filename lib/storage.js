...
export function storageReport(runningIds = new Set()) {
  ...
  const workspaces = items.map((w) => {
    ...
    const partUploads = [];
    let mediaFiles = [];
    try { mediaFiles = fs.readdirSync(mediaDir); } catch { /* no media yet */ }
    for (const f of mediaFiles) {
      const size = fileSize(path.join(mediaDir, f));
      mediaBytes += size;
    ...
    return {
      ...
      totalBytes: dirSize(dir), mediaBytes, originalBytes,
      narrationCacheBytes: dirSize(path.join(rendersDir, 'narration-cache')),
      tmp, partUploads, renders,
    };
  });
  return { disk: diskFree(), dataDirBytes: dirSize(DATA_DIR), workspaces };
}
...
export function cleanupStorage(runningIds = new Set()) {
  ...
  for (const w of listWorkspaces().items) {
    ...
    const mediaDir = path.join(wsDir(w.id), 'media');
    let mediaFiles = [];
    try { mediaFiles = fs.readdirSync(mediaDir); } catch { /* no media yet */ }
    for (const f of mediaFiles) {
  ...
  return { freedBytes, removedTmp, removedParts };
}
