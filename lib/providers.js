...
export async function heygenGenerate({ avatarId, avatarKind = 'avatar', voiceId, text, texts, title, orientation = 'portrait', speed }) {
  const portrait = orientation === 'portrait';
  // Long on-camera sections arrive pre-chunked as `texts`; each chunk becomes
  // a scene of the same video, so one video_id still comes back.
  const inputs = (texts?.length ? texts : [text]).map((t) => ({
    character: avatarKind === 'talking_photo'
      ? { type: 'talking_photo', talking_photo_id: avatarId }
      : { type: 'avatar', avatar_id: avatarId, avatar_style: 'normal' },
    voice: { type: 'text', voice_id: voiceId, input_text: t, ...(speed && speed !== 1 ? { speed } : {}) },
  }));
