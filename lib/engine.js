...
{
  "faq": [{"q": "conversational question exactly as someone would ask an AI assistant", "a": "45-70 word answer-first response with a number or named entity"}],  // exactly 4 pairs; at least one value question ("Is X worth it?", "What do you get for the price?") answered only by what the creator provides
...
export async function generatePackage({ profile, topic, angle, pillar, series, media = [], platformIds, ctaUrl, onProgress }) {
  ...
  const pkg = {
    ...
    // Publish Run needs to know which attached assets are stills vs video
    // without re-fetching the whole (now shared, hundreds-of-items) library,
    // same reason altTexts is carried on the package instead of looked up.
    mediaKinds: Object.fromEntries(media.map((m) => [m.id, m.kind])),
  ...
  try {
    ...
    // The answer layer failing used to be invisible: the package just came
    // back with template FAQ and an empty query map and nothing said why.
    // Record it so the next failure explains itself (same lesson as
    // chapterTitleError), and the UI can offer the rebuild.
    pkg.faq = templateFaq(topic, profile);
    pkg.answerLayerError = String(err.message || err).slice(0, 300);
  }
...
function templateFaq(topic, profile) {
  ...
  return [
    ...
    { q: `How much does ${topic} cost?`, a: '[FILL: concrete numbers, the specific figures AI engines cite]' },
    { q: `Is ${topic} worth it?`, a: '[FILL: verdict-first answer with one lived example]' },
  ];
}

// Citation-layer regenerate: rebuilds the AI-answer metadata (query map,
// FAQ, cite lines, keywords, entities) from the package's FINISHED copy,
// so hand-edited facts (prices, addresses, dates) ground the answers.
// Platform fields are never touched; FAQ pairs whose answers are already
// real (no [FILL]) are kept and only the gaps regenerate.
function packageDigest(pkg) {
  const text = (v) => (v == null ? '' : Array.isArray(v) ? v.join('\n') : String(v));
  const parts = [`TOPIC: ${pkg.topic}`];
  if (pkg.definition) parts.push(`DEFINITION: ${pkg.definition}`);
  const grab = [
    ['youtube_long', ['title', 'description']],
    ['linkedin', ['article']],
    ['gbp', ['post']],
    ['bing', ['description']],
    ['newsletter', ['body']],
  ...
  for (const [pid, keys] of grab) {
    for (const key of keys) {
      const v = text(pkg.platforms?.[pid]?.fields?.[key]);
      if (v) parts.push(`--- ${pid}.${key} ---\n${v.slice(0, pid === 'linkedin' ? 4000 : 1500)}`);
    }
  }
  return parts.join('\n\n').slice(0, 16000);
}

export async function regenerateCitations({ profile, pkg }) {
  const status = providerStatus();
  if (!status.anthropic) throw new Error('the citation layer needs the Claude key (ANTHROPIC_API_KEY)');
  const meta = await claudeJson({
    system: masterContext(profile),
    messages: [{
      role: 'user',
      content: `${FAQ_PROMPT(pkg.topic)}

GROUND TRUTH — the package's finished, hand-edited copy. Use ONLY facts that appear below or in the creator context (prices, dates, addresses, counts are all in there). If a fact genuinely appears nowhere, leave a [FILL: ...] placeholder instead of inventing it.

The source copy below may itself contain comparisons or contrasts against booking sites, search engines, or other advisors. Do NOT carry those across. Take the underlying fact and state it as what the creator provides. ${NO_COMPARISON_RULE}
...
// Add platform assets to a package that already exists. A package is a
// living thing: media gets attached later, the answer layer gets rebuilt,
// copy gets hand-edited. Platforms were the one part fixed at generation,
// which forced a whole new package (and a split registry and score) just to
// reach one more surface. Existing assets, approvals, published URLs, and
// every hand edit are left untouched; only the new platforms are written.
export async function generatePlatforms({ profile, pkg, platformIds, media = [], onProgress }) {
  const status = providerStatus();
  if (!status.anthropic) throw new Error('adding platforms needs the Claude key (ANTHROPIC_API_KEY)');
  const pillar = (profile.pillars || []).find((p) => p.id === pkg.pillarId) || null;
  const series = (profile.series || []).find((s) => s.id === pkg.seriesId) || null;
  const ids = (platformIds || []).filter((id) => PLATFORMS[id] && !pkg.platforms?.[id]);
  if (!ids.length) throw new Error('those platforms are already in this package (or are not real platforms)');

  const system = masterContext(profile, { pillar, series, media, ctaUrl: pkg.ctaUrl });
  // Ground the new assets in the copy that already exists, so a package
  // speaks with one voice across surfaces added weeks apart.
  const anchor = `\n\nThis package already exists and these assets are live or approved. Stay consistent with the facts, framing, and voice below. Never contradict them, and never reuse whole sentences verbatim: each platform gets native copy.\n\n${packageDigest(pkg)}`;
  ...
  for (const id of ids) {
    ...
    try {
      const fields = await claudeJson({
        system,
        messages: [{ role: 'user', content: platformPrompt(spec, pkg.topic, pkg.angle) + anchor }],
        maxTokens: id === 'youtube_long' || id === 'linkedin' ? 8000 : 3000,
      });
      out[id] = { fields };
    } catch (err) {
      out[id] = { fields: templateAsset(spec, pkg.topic, profile).fields, error: String(err.message).slice(0, 200) };
    }
    done += 1;
    onProgress?.({ platform: id, done, total: ids.length });
  }
  return out;
}

// Reshare commentary: when a brand publishes from a person and amplifies
// from its company page, a bare reshare underperforms and reads lazy. The
// second post needs its own short framing in the BRAND's voice (third
// person about the author) rather than a copy of the personal post.
export async function writeReshareComment({ profile, pkg, platformId }) {
  const biz = profile.business || {};
  const person = biz.person?.name || 'our founder';
  const fields = pkg.platforms?.[platformId]?.fields || {};
  const title = text(fields.article_title) || pkg.topic;
  const fallback = [
    `${person} on ${String(pkg.topic).replace(/\?$/, '').toLowerCase()}.`,
    pkg.definition ? pkg.definition : '',
    'Full article linked below.',
  ].filter(Boolean).join(' ').slice(0, 600);

  if (!providerStatus().anthropic) return { text: fallback, mode: 'template' };
  try {
    const out = await claude({
      ...
      messages: [{
        role: 'user',
        content: `${biz.name} is resharing this article from its company page. ${person} published it from their personal profile; the company page now amplifies it.

Write ONLY the reshare commentary: 2 to 3 sentences, 40 to 60 words, in the company's voice speaking about ${person} in the third person. Say why this matters to the reader and what they will get from reading it. Do not repeat the article's opening line, do not use hashtags, do not add a link (the share carries it), never use em dashes or en dashes, and never compare the business to or characterize any booking site, platform, agency, supplier, or other advisor.

Article title: ${title}
${pkg.definition ? `Definition: ${pkg.definition}` : ''}
${pkg.quotable ? `Quotable line: ${pkg.quotable}` : ''}`,
      }],
    });
    const clean = String(out || '').replace(/[—–]/g, ',').replace(/^["']|["']$/g, '').trim();
    return clean ? { text: clean.slice(0, 700), mode: 'ai' } : { text: fallback, mode: 'template' };
  } catch (err) {
    return { text: fallback, mode: 'template', error: String(err.message || err).slice(0, 200) };
  }
}
...
// The fallback used to be silent (bare catch, no reason kept anywhere),
// which meant every AI-selection failure looked identical to "there was
// never a Claude key" from the caller's side. It carries an error message
// on fallback now, same pattern as carouselPlan.error and
// answerLayerError, so a production failure is diagnosable instead of
// just quietly downgrading every package to keyword matching.
export async function selectMedia({ profile, topic, angle, pillar, items, count = 8 }) {
  ...
  const heuristic = (error) => {
    ...
    return { ids: scored.slice(0, count).map((s) => s.m.id), reasons: {}, mode: 'heuristic', ...(error ? { error } : {}) };
  };

  if (!status.anthropic) return heuristic();

  // The shared library can run into the hundreds of items now that it is
  // cross-workspace, so the catalog is capped at 200 entries, but each
  // entry (caption + keywords) is long enough that a verbose reply for
  // count=12 has run the model close to the old 1500-token budget; give it
  // real headroom and the same one-retry-on-truncation resilience every
  // other JSON-generating call in this file already gets.
...
Select for: topical relevance, scroll-stopping visual strength, story-arc coverage (a hook shot, proof shots, a human moment, a closer), and a mix of images plus at least one video if any exist. Keep each reason short, 8-15 words. Respond with ONLY JSON:
{"selections": [{"id": "media id from the brackets", "reason": "8-15 words: why this asset + where to use it"}]}`,
    ...
    return heuristic(`AI returned ${valid.length} usable picks, needed ${Math.min(3, usable.length)}`);
  } catch (err) {
    return heuristic(String(err.message || err).slice(0, 300));
  }
}
...
function parseCarouselSlides(raw) {
  ...
  // Two header shapes, kept strict so a stray "15+ years" or "2026 was"
  // never reads as a slide break:
  //   LABELLED  "Slide 1:", "Slide 1 (cover):", or a bare "SLIDE 2 (Definition)"
  //             header line with the body on the lines beneath it
  //   NUMBERED  "1.", "2)", "3 -" with the body on the same line
  const LABELLED = /^\s*slide\s*(\d+)\s*(?:\(([^)]*)\))?\s*[\.\):\-]?\s*(.*)$/i;
  const NUMBERED = /^\s*(\d+)\s*(?:\([^)]*\))?\s*[\.\):\-]\s+(.*)$/;
  const slides = [];
  let cur = null;
  for (const line of lines) {
    const lab = line.match(LABELLED);
    const num = lab ? null : line.match(NUMBERED);
    if (lab || num) {
      // A labelled header often carries its theme in the parenthetical and
      // nothing else, so seed the text with it and let the body follow.
      const seed = lab
        ? [lab[2], lab[3]].filter(Boolean).join(' ').trim()
        : (num[2] || '').trim();
      cur = { n: +(lab ? lab[1] : num[1]), text: seed };
      slides.push(cur);
...
export async function matchCarouselSlides({ profile, pkg, items }) {
  ...
  const clamp = (s) => {
    ...
    const cut = t.slice(0, 125);
    return cut.slice(0, cut.lastIndexOf(' ') > 80 ? cut.lastIndexOf(' ') : 125).trim();
  };
  const heuristic = () => {
    const taken = new Set();
    return slides.map((s) => {
...
{"slides": [{"n": 1, "id": "media id from the brackets", "reason": "10-18 words on why this image ranks for this slide", "alt": "entity-rich alt text for this slide <= 125 chars"}]}`,
    ...
    throw new Error(`AI returned ${valid.length} usable picks for ${slides.length} slides`);
  } catch (err) {
    // Matching must never fail the request, but the fallback reason has to
    // survive somewhere readable (same lesson as chapterTitleError).
    console.warn(`carousel match fell back to heuristic: ${err.message}`);
    return { slides: heuristic(), mode: 'heuristic', error: String(err.message || err).slice(0, 300) };
  }
}
